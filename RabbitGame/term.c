...
#ifdef UNIX
	return getmaxx(stdscr) - getbegx(stdscr);
#endif
...
#ifdef UNIX
	return getmaxy(stdscr) - getbegy(stdscr);
#endif
...
#ifdef UNIX
	mvprintw(y + getbegy(stdscr), x + getbegx(stdscr), "%c", c);
	refresh();
#endif
...
void term_puts(unsigned x, unsigned y, const char *s)
{
	...
	if (y * term_xmax() + x + len >= term_ymax() * term_xmax()) {
		term_destruct();
		fprintf(stderr,
                "term_puts had no room to print string of length %lu\n"
	...
	for (p = s; *p != '\0'; ++p) {
		if (!isprint((unsigned char)*p)) {
			term_destruct();
			fprintf(stderr,
                    "unprintable char %u at subscript %lu "
                    "in term_puts(%u, %u)\n",
...
#ifdef UNIX
	mvprintw(y + getbegy(stdscr), x + getbegx(stdscr), "%s", s);
	refresh();
#endif
...
#ifdef UNIX
	refresh();
	return A_CHARTEXT & mvinch(y + getbegy(stdscr), x + getbegx(stdscr));
#endif
